...
impl SketchArrayFile {
    ...
    pub fn read_all(filename: &str, number_bins: usize) -> Vec<u64> {
        ...
        let mut reader = BufReader::new(File::open(filename).expect(&format!("Could not read from {filename}")));
    ...
    pub fn read_batch(filename: &str, sample_indices: &[usize], sample_stride: usize) -> Vec<u64> {
        // Just stream the whole file and convert to u64 vec
        let mut mmap = Self::memmap_file(filename).expect(&format!("Could not memory map {filename}"));
        let mut buffer = [0u8; mem::size_of::<u64>()];
        let mut flat_sketch_array: Vec<u64> = Vec::with_capacity(sample_stride * sample_indices.len());
        ...
        for sample_idx in sample_indices {
            for bin_idx in 0..sample_stride {
                ...
                let bin_val = u64::from_le_bytes(*array_ref![mmap, start_byte, mem::size_of::<u64>()]);
