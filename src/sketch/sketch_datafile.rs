...
impl SketchArrayReader {
    ...
    ///
    /// `total_number_bins` is an optional hint to set the buffer size
    pub fn read_all_from_skq(&mut self, total_number_bins: Option<usize>) -> Vec<u16> {
        // Storage
        let mut flat_sketch_array: Vec<u16> = if let Some(known_capacity) = total_number_bins {
            Vec::with_capacity(known_capacity)
        } else {
            Vec::new()
        };
