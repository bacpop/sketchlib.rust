    ...
    if let Some(mmap) = input_reader.mmap_file() {
        for &sample_idx in sample_indices {
            ...
            for bin_idx in 0..input_reader.sample_stride {
                let start_byte = (sample_idx * input_reader.sample_stride + bin_idx) * mem::size_of::<u64>();
                let bin_val =
...
impl SketchArrayWriter {
    ...
    pub fn new(filename: &str,
        ...
        let mmap_file = if mmap {
            Some(memmap_file(filename).unwrap_or_else(|_| panic!("Could not memory map {filename}")))
        } else {
            None
        };
        Self {
...
impl SketchArrayReader {
    ...
    pub fn read_sketch_from_skq(&mut self, sample_idx: usize) -> Result<Vec<u16>, Error> {
        ...
        self.sketch_reader.seek(std::io::SeekFrom::Start((sample_idx * self.sample_stride * buffer.len()) as u64))?;
        ...
        if let Some(mmap_array) = &self.mmap_file {
            for sample_idx in sample_indices {
                for bin_idx in 0..sample_stride {
                    ...
                    let bin_val =
                        u64::from_le_bytes(*array_ref![mmap_array, start_byte, mem::size_of::<u64>()]);
